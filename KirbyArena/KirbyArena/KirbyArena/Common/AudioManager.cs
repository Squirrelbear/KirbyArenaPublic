...
namespace KirbyArena
{
    public class AudioManager
    {
        ...
        private KirbyGame appRef;
        private InputManager inputManager;

        public AudioManager(List<string> songPaths, KirbyGame appRef)
            : this(songPaths, MusicMode.Sequence, -1, appRef)
        {
        }

        public AudioManager(List<string> songPaths, MusicMode mode, KirbyGame appRef)
            : this(songPaths, mode, -1, appRef)
        {
        }

        public AudioManager(List<string> songPaths, MusicMode mode, int startSongID, KirbyGame appRef)
        {
            this.appRef = appRef;
        ...
        public void update(GameTime gameTime)
        {
            ...
            if (inputManager.isKeyPressed(Keys.OemMinus))
            {
                if (MediaPlayer.Volume >= 0.0)
                    setVolume(MediaPlayer.Volume -= 0.05f);
            }
        ...
        public void setMute(bool ismute)
        {
            ...
            appRef.getGameOptions().mute = getMuted();
            appRef.getGameOptions().saveFileDefaults();
            /*if (ismute)
            {
                MediaPlayer.Pause();
            }
        ...
        public void setVolume(float volume)
        {
            ...
            appRef.getGameOptions().volume = getVolume();
            appRef.getGameOptions().saveFileDefaults();
        }
